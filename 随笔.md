...
> 生命周期考虑：localStorage 的生命周期是永久的，关闭浏览器之后，localStorage 数据不会消失 sessionStorage 的生命周期仅在当前会话下有效
> cookie 存储大小是 4KB localStorag \ sessionStorage 是 5MB
> localStorage \ sessionStorage 都保存在客户端，不与服务器进行交互通信
> localStorage \ sessionStorage 只能存储字符串类型，对于复杂类型，可以用 ECMAScript 提供的 JSON 对象的 string 和 parse 来处理
...
```
> 重绘跟回流
> 重绘： 当元素样式改变不影响布局时，浏览器将使用重绘对元素更新，此时由于只需要ui层面的重新像素绘制，由此损耗较少
> 常见的重绘操作有：
#### 1、改变元素颜色
#### 2、改变元素背景色
